...
go test -v -tags "synapse_blacklist" -count=1 "${extra_test_args[@]}" -run "TestRestrictedRoomsRemoteJoinFailOverInMSC3787Room" -json "${test_packages[@]}"| tee "test_results.log.jsonl"
set -o pipefail
